...
// 게시글 수정 모달에 필요한 props 타입 정의
interface PostEditModalProps {
  post: { id: number; title: string; content: string }; // 수정할 게시글 정보
  onClose: () => void; // 모달을 닫는 함수
  onEdit: (id: number, title: string, content: string) => void; // 게시글 수정 함수
}
...
export default function PostEditModal({
  ...
  // 제목 입력값 상태 (초기값: 기존 게시글 제목)
  const [title, setTitle] = useState(post.title);
  // 내용 입력값 상태 (초기값: 기존 게시글 내용)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsSubmitting(true); // 수정 중 상태로 변경
    onEdit(post.id, title, content); // 상위에서 전달받은 게시글 수정 함수 호출
    setIsSubmitting(false); // 수정 중 상태 해제
    onClose(); // 모달 닫기
  };
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              type="submit"
              disabled={isSubmitting} // 수정 중일 때 비활성화
